...
import com.baidu.fengchao.user.UserUtils;
...
//            bulkRequest.add(client.prepareIndex("comment_index_" + indexNum, "comment_ugc").setSource(indexMap));
            bulkRequest.add(client.prepareIndex("comment_index", "comment_ugc").setSource(indexMap));
        }
    ...
    private static void fillToES(long userid, List<Map<String, Object>> words)
            throws IOException {
        ESClientColumn.buildIndexBatch(client, words);
    }
    ...
    public static Map<String, Object> convertToModel(String[] fields) {
        ...
        //
//        for (int i = 1; i <= 31; i++) {
//            model.put("pay_" + DateUtils.getBackDay(i), RandomUtils.randomBoolean() ? RandomUtils.nextint(1, 1000) : "");
//            model.put("click_" + DateUtils.getBackDay(i), RandomUtils.randomBoolean() ? RandomUtils.nextint(1, 1000) : "");
//            model.put("show_" + DateUtils.getBackDay(i), RandomUtils.randomBoolean() ? RandomUtils.nextint(1, 1000) : "");
//        }
        Date startDate = null;
        try {
            startDate = DateUtils.parseDate("2014-08-01", "yyyy-MM-dd");
        ...
        if (RandomUtils.nextInt(0, 3) == 0) {
            int end = RandomUtils.nextInt(0, 30);

            for (int i = 0; i < end; i++) {
                Date d = DateUtils.addDays(startDate, i);
                Integer show = RandomUtils.nextInt(0, 1000);
                Integer clk = RandomUtils.nextInt(0, show);
                Double pay = RandomUtils.nextDouble(0, 100D) * clk;
                model.put("show_" + dateFormat.format(d) + "_i", show);
                model.put("clk_" + dateFormat.format(d) + "_i", clk);
                model.put("pay_" + dateFormat.format(d) + "_d", pay);
            }
        }
        return model;
    }
